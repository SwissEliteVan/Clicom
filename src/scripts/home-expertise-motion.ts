  ...
  const context = gsap.context(() => {
    ...
    media.add('(prefers-reduced-motion: no-preference)', () => {
      ...
      if (build) {
        ...
        const timeline=gsap.timeline({paused:true}).fromTo(grid,{strokeDashoffset:1},{strokeDashoffset:0,stagger:.025,duration:.5,ease:'power1.out'}).to(blocks[0],{clipPath:'inset(0 0% 0 0)',duration:.42,ease:'power2.out'},.22).to(rules,{scaleX:1,stagger:.07,duration:.28},.32).to(button,{opacity:1,scale:1,duration:.25,ease:'back.out(1.6)'},.62).to(route,{strokeDashoffset:0,duration:.6,ease:'power1.inOut'},.66).to(signal,{x:460,y:-105,duration:.65,ease:'power2.inOut'},.7).to(button,{scale:1.06,duration:.12,yoyo:true,repeat:1},1.25).to(pulse,{scaleX:1,stagger:.1,duration:.3},1.2);
        ScrollTrigger.create({trigger:build,start:'top 76%',once:true,onEnter:()=>timeline.play()});
        if(matchMedia('(pointer:fine)').matches){const moveX=gsap.quickTo(signal,'x',{duration:.35}),moveY=gsap.quickTo(signal,'y',{duration:.35});listen(build,'pointermove',((event:PointerEvent)=>{const box=build.getBoundingClientRect();moveX(440+(event.clientX-box.left-box.width/2)*.025);moveY(-100+(event.clientY-box.top-box.height/2)*.025)}) as EventListener);listen(build,'pointerleave',(()=>{moveX(460);moveY(-105)}) as EventListener)}
      }
      ...
      if (visibility) {
        ...
        const timeline=gsap.timeline({paused:true}).from(fragments,{scale:.45,opacity:0,stagger:{each:.018,from:'random'},duration:.32}).to(lines,{strokeDashoffset:0,stagger:.025,duration:.5},.18).to(relevant,{x:index=>(index%2?-1:1)*(35+index*3),y:index=>(index%3-1)*24,color:'rgba(247,249,252,.72)',duration:.65,ease:'power2.inOut'},.35).to(core,{scale:1,duration:.55,ease:'back.out(1.4)'},.55);
      ...
      if (acquisition) {
        ...
        const timeline=gsap.timeline({paused:true}).to(incoming,{strokeDashoffset:0,stagger:.045,duration:.42,ease:'power2.in'}).fromTo(dots,{x:-35,opacity:0},{x:0,opacity:1,stagger:{each:.025,from:'random'},duration:.28},.08).to(dots.filter((_,index)=>index%3!==0),{opacity:.08,scale:.4,duration:.25},.46).to(outgoing,{strokeDashoffset:0,stagger:.08,duration:.4,ease:'power2.out'},.52).to(quality,{opacity:1,scale:1.35,stagger:.1,duration:.18,ease:'back.out(2)'},.7).to(quality,{scale:1,duration:.16},.92);
        ScrollTrigger.create({trigger:acquisition,start:'top 76%',once:true,onEnter:()=>timeline.play()});
        if(matchMedia('(pointer:fine)').matches)listen(acquisition,'pointerenter',(()=>gsap.fromTo(outgoing,{strokeDashoffset:1,strokeWidth:1},{strokeDashoffset:0,strokeWidth:3,stagger:.08,duration:.55,ease:'power2.out'})) as EventListener);
      }
