...
function initialiseWork(root:HTMLElement){
  const context=gsap.context(()=>{
    ...
    media.add('(min-width: 961px) and (prefers-reduced-motion: no-preference)',()=>{
      ...
      gsap.timeline({scrollTrigger:{trigger:primary,start:'top bottom',end:'bottom top',scrub:.8}})
      ...
      gsap.timeline({scrollTrigger:{trigger:'.work-transition',start:'top 78%',end:'bottom 35%',scrub:.7}})
      ...
      gsap.timeline({scrollTrigger:{trigger:secondary,start:'top bottom',end:'bottom top',scrub:.8}})
